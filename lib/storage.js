...
export function loadStore() {
  ...
  try {
    ...
    // Saves whose test was deleted before deleteResult cleaned up after itself.
    // They were unlabelable on the Saved page (the filter showed a bare "—"),
    // so clear them once; from now on deleting a result takes its saves with it.
    if (!parsed.savedOrphansV1) {
      const live = new Set([
        ...out.profiles.flatMap((p) => (p.history || []).map((h) => h.id)),
        ...out.guestHistory.map((h) => h.id),
      ]);
      out.saved = out.saved.filter((s) => !s.resultId || live.has(s.resultId));
      out.savedOrphansV1 = true;
    }
    return out;
  } catch {
    return { ...DEFAULTS };
