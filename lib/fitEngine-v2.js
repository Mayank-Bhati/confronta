...
function singleGoalFit(course, goal, t) {
  ...
  if (goal === "work") {
    ...
    note = course.handsOn >= 4 ? t("fo_work_hands") : t("fo_work_theory");
  } else if (goal === "salary") {
    score = Math.round(Math.min(100, (course.salary / 40000) * 100));
    note = t("fo_salary", { n: course.salary.toLocaleString() });
  } else {
    score = course.mastersAccess * 20;
    note = course.mastersAccess >= 4 ? t("fo_study_yes") : t("fo_study_rare");
  }
...
export function fitEnvironment(course, prefs, homeCity, t = idT) {
  ...
  if (course.env.dropout >= 22) reasons.push(t("env_dropout", { p: course.env.dropout }));
  else reasons.push(t("env_again", { p: course.env.wouldChooseAgain }));
  ...
  if (prefs.monthlyBudget) {
    ...
    if (over <= 0) {
      reasons.push(t("env_budget_fit", {
        total: est.total.toLocaleString(), fees: est.fees, living: est.living,
        budget: prefs.monthlyBudget.toLocaleString(),
      }));
    } else {
      const hint = (prefs.isee || "mid") === "low" ? t("env_budget_dsu") : t("env_budget_sch");
      reasons.push(t("env_budget_over", { total: est.total.toLocaleString(), over: over.toLocaleString() }) + hint);
    }
  }
...
export function prepList(course, profile, t = idT) {
  ...
  if (course.env.dropout >= 22) items.push(t("prep_drop"));
  return items;
}
